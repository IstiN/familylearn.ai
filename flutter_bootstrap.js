...
_flutter.loader.load({
  // Disable the default blue loading bar at the top of the page during the loading phase
  loadingIndicator: {
    enabled: false,
  },
  // Skipping serviceWorker configuration here to avoid the 4000ms "prepareServiceWorker" timeout.
  // The service worker will still be registered by the browser if it was previously installed,
  // but Flutter won't wait for it to be "ready" before starting the app.
  onEntrypointLoaded: async function(engineInitializer) {
    const engine = await engineInitializer.initializeEngine({
      // Move configuration here as suggested by the deprecation warning
      canvasKitBaseUrl: "/canvaskit/",
      // Disable the blue loading bar during engine/CanvasKit initialization
      loadingIndicator: {
        enabled: false,
      },
    });

    // Remove loading indicator just before running the app
